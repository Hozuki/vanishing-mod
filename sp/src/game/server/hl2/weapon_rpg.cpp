...
void CMissile::ShotDown(void) {
	...
	IGameEvent *event = gameeventmanager->CreateEvent("van_rpg_gunship_missed");
	if (event) {
...
#define _VANISHING_FIRE_RPG_GUNSHIP_MISSED_EVENT() \
	do { \
		IGameEvent *event = gameeventmanager->CreateEvent("van_rpg_gunship_missed"); \
		if (event) { \
			gameeventmanager->FireEvent(event); \
	...
	if (iEntity < 0) {
		...
		_VANISHING_FIRE_RPG_GUNSHIP_MISSED_EVENT();
	} else {
...
#undef _VANISHING_FIRE_RPG_GUNSHIP_MISSED_EVENT
#endif
...
void CWeaponRPG::Precache(void) {
	...
	// Laser dot...
	PrecacheModel("ssprites/reddot.vmt");
